...
@section('conteudo')
    <div class="container-fluid px-4">
        ...
        <ol class="breadcrumb mb-4">
            <li class="breadcrumb-item active">Listagem de Tarefas</li>
        </ol>
        <div class="card mb-4">
            ...
            <div class="card-body">
                ...
                <table id="datatablesSimple">
                    ...
                    <tbody>
                        @foreach ($tarefa as $linha)
                            <tr>
                                <td>{{ $linha->id }}</td>
                                <td>{{ $linha->tarefa_titulo }}</td>
                                <td>{{ $linha->projeto->projeto_nome }}</td>
                                <td>
                                    @foreach ($linha->colaborador as $colaborador)
                                        @foreach ($usuarios as $usuario)
                                            @if ($colaborador->usuario_id == $usuario->id)
                                                {{ $usuario->usuario_nome }} <br>
                                            @endif
                                        @endforeach
                                    @endforeach
                                </td>
                                <td>{{ $linha->tarefa_descricao }}</td>
                                <td>{{ $linha->tarefa_status }}</td>
                                <td>{{ $linha->tarefa_data_conclusao }}</td>

                                <td>

                                    <a href="{{ route('tarefa_alterar', ['id' => $linha->id]) }}"
                                    ...
                                    <a href="{{ route('tarefa_excluir', ['id' => $linha->id]) }}"
            ...
            <div class="modal-content">
                ...
                <form action="/tarefa" method="POST">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Cadastro de nova Tarefa</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control"  name="tarefa_titulo">
                            <label for="floatingInput">Titulo da tarefa</label>
                        </div>
